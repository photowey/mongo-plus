...
/**
 * SelectOneMethod - Built-in selectOne, registered as MapperMethod.
 *
 * @author photowey
 * @version 2026.1.0.0
 * @since 2026/03/09
 */
public final class SelectOneMethod extends AbstractMethod {

    public static final String METHOD_NAME = "selectOne";
        ...
        return new WrapperHandler() {
            ...
            protected <T> Object execute(ExecutorGetter getter, Class<T> entityClass, AbstractWrapper<T> wrapper) {
                if (wrapper instanceof LambdaQueryWrapper) {
                    return getter.queryExecutor().selectOne((LambdaQueryWrapper<T>) wrapper, entityClass);
                }
                if (wrapper instanceof QueryWrapper) {
                    return getter.queryExecutor().selectOne((QueryWrapper<T>) wrapper, entityClass);
                }
                return getter.queryExecutor().selectOne(wrapper, entityClass);
            }
        };
    }
}
