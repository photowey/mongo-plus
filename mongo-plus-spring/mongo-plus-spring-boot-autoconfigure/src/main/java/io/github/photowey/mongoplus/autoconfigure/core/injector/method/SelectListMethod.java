...
import io.github.photowey.mongoplus.autoconfigure.core.proxy.handler.WrapperHandler;
...
/**
 * SelectListMethod - Built-in selectList, registered as MapperMethod.
 *
 * @author photowey
 * @version 2026.1.0.0
 * @since 2026/03/09
 */
public final class SelectListMethod extends AbstractMethod {

    public static final String METHOD_NAME = "selectList";
    ...
    protected MethodHandler createHandler() {
        return new WrapperHandler() {
            @Override
            protected <T> Object execute(ExecutorGetter getter, Class<T> entityClass, AbstractWrapper<T> wrapper) {
                if (wrapper instanceof LambdaQueryWrapper) {
                    return getter.queryExecutor().selectList((LambdaQueryWrapper<T>) wrapper, entityClass);
                }
                if (wrapper instanceof QueryWrapper) {
                    return getter.queryExecutor().selectList((QueryWrapper<T>) wrapper, entityClass);
                }
                return getter.queryExecutor().selectList(wrapper, entityClass);
            }
        };
    }
}
